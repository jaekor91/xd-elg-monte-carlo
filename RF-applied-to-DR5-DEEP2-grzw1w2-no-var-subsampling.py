...
total_try  = 25
max_depths = np.random.uniform(low=7, high=17, size=total_try).astype(int)
...
for i, max_depth in enumerate(max_depths):
    ...
    for train, test in kf.split(X):
        ...
        rnd_clf = RandomForestClassifier(n_estimators=N_estimators, max_leaf_nodes=max_leave, max_depth=max_depth, n_jobs=-1, max_features=None)
    ...
    Precision_mean = np.mean(results_arr[i, 2, :])
    Precision_std = np.std(results_arr[i, 2, :])
...
np.save("RF-applied-DEEP2-F234-grzw1w2-no-var-sub.npy", results_arr)
...
# Best depth/max leaves: 8, 843
# 0.765223305138 1889.73919904


# Max depths/leaves: 8/843
# CV: Ndensity mean/std: 1889.7/117.4
# CV: Ndensity Raw mean/std: 1867.2/98.5
# CV: Precision mean/std: 0.765/0.020


# F2: Ndensity mean/std: 1531.6/16.3
# F2: Ndensity Raw mean/std: 1519.1/21.3
# F2: Precision mean/std: 0.850/0.005


# F3: Ndensity mean/std: 1920.3/30.3
# F3: Ndensity Raw mean/std: 1913.6/18.9
# F3: Precision mean/std: 0.909/0.006


# F4: Ndensity mean/std: 2829.4/45.7
# F4: Ndensity Raw mean/std: 2635.9/29.8
# F4: Precision mean/std: 0.883/0.005


# F234: Ndensity mean/std: 2036.2/24.5
# F234: Ndensity Raw mean/std: 1977.3/15.3
# F234: Precision mean/std: 0.885/0.004                
...
# plt.scatter(max_depths, precision_mean_list)
# plt.show()
plt.close()
...
# Construct and train a new classifier
N_estimators = 100
rnd_clf = RandomForestClassifier(n_estimators=N_estimators, max_leaf_nodes=max_leaves[idx_best], max_depth=max_depths[idx_best], n_jobs=-1, max_features=None)
...
# Last probability threshold: 0.663
# Variable importance
# g: 0.235
# r: 0.471
# z: 0.167
