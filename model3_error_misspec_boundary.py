...
if True:
    ...
    cell_select_fiducial_centers = instance_model.cell_select_centers()
    ...
    for i, b in enumerate(bands):
        ...
        for j, dm in enumerate(dm_list):
            ...
            for k in range(3):
                cv_tag = "adpat-vs-fixed-%s-%s%d" % (b, str_sign(dm), np.abs(dm*100))
                instance_model.gen_select_boundary_slices(slice_dir = k, model_tag=fdir+"model3", cv_tag=cv_tag,\
                    var_x_ext = cell_select_fiducial_centers[:, 0], var_y_ext = cell_select_fiducial_centers[:, 1], gmag_ext = cell_select_fiducial_centers[:, 2],\
...
if True:
    ...
    for i, b in enumerate(bands):
        ...
        for j, dm in enumerate(dm_list):
            ...
            print "Generate the new selection"
            start = time.time()
            ...
            print "Save the new selection."
            cell_select_new = instance_model2.cell_select
            cell_select_new_centers = instance_model2.cell_select_centers()
            ...
            print "/---- Plotting boundary ----/"
            for k in range(3):
                cv_tag = "base-vs-misspec-%s-%s%d" % (b, str_sign(dm), np.abs(dm*100))
                instance_model.gen_select_boundary_slices(slice_dir = k, model_tag=fdir+"model3", cv_tag=cv_tag,\
                    var_x_ext = cell_select_new_centers[:, 0], var_y_ext = cell_select_new_centers[:, 1], gmag_ext = cell_select_new_centers[:, 2],\
                    use_parameterized_ext=True, plot_ext =True, alpha_ext=0.3, guide=True)
