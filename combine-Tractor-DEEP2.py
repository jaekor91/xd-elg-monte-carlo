...
for i, fnum in enumerate([2, 3, 4]):
    ...
    ibool = (tycho == 0)
    print("Apply Tycho-2 mask.\n")
    ra_d2, dec_d2, tycho, B, R, I, cn, w = load_DEEP2(d2_dir + ("deep2-f%d-photo-redz-oii.fits" % fnum), ibool=ibool)
    deep2 = load_fits_table(d2_dir + ("deep2-f%d-photo-redz-oii.fits" % fnum))[ibool]
    ...
    ##############################################################################
    print("2. Cross-matching Tractor objects with g<%.1f and DEEP2 objects within each field. Check astrometric difference." % glim) 
