...
for i, targets in enumerate([targets1, targets2, targets3]):
    ...
    for j in range(ra_tmp.size):
        line = ",".join([str(x) for x in [bit_mask[j], ra_tmp[j], dec_tmp[j], g_tmp[j], 2]])
        line += "\n"
        f.write(line)
    
    # Add stars
    for j in range(ra_stars.size):
        line = ",".join([str(x) for x in [2**1, ra_stars[j], dec_stars[j], gmag_stars[j], 1]])
        line += "\n"
        f.write(line)        
    ...
    for j in range(ra_gal.size):
        line = ",".join([str(x) for x in [2**2, ra_gal[j], dec_gal[j], gmag_gal[j], 1]])
        line += "\n"
        f.write(line)        
...
for j in range(ra.size):
    line = ",".join([str(x) for x in ["ELG-" + str(redz[j]), ra[j], dec[j], gmag[j], 2]])
    line += "\n"
    f.write(line)
